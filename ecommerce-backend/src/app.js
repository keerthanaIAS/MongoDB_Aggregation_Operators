 ...
 * Shows practical differences between operators
 */
app.get("/operators/all", async (req, res) => {
    res.json({
        success: true,
        message: "Complete MongoDB Operators Reference",
        
        comparison_operators: {
            "$eq": "Equal to (same as field: value)",
            "$ne": "Not equal to",
            "$gt": "Greater than",
            "$gte": "Greater than or equal",
            "$lt": "Less than",
        ...
        practical_examples: {
            find_premium_usa: {
                query: '{ country: "USA", membership: "premium" }',
                or_using_operators: '{ $and: [{country: "USA"}, {membership: "premium"}] }'
            },
            find_multiple_countries: {
                query: '{ country: { $in: ["USA", "UK", "Japan"] } }',
                note: "Better than multiple $or conditions"
            },
            find_price_range: {
                query: '{ $and: [{price: {$gte: 100}}, {price: {$lte: 1000}}] }',
                note: "Need $and because both conditions are on SAME field"
            },
            find_with_conditions: {
                query: '{ $or: [{status: "cancelled"}, {totalAmount: {$gt: 1500}}] }',
                note: "$or when ANY condition match is okay"
            },
            find_array_contains: {
                all: '{ tags: { $all: ["tech", "gaming"] } }',
                in: '{ tags: { $in: ["tech", "gaming"] } }',
                difference: "$all requires BOTH, $in requires ANY"
            }
        },
